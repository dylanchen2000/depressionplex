...
>
> 27 行里**正好有 1 行**是这种：`30mg_2周-ch4`，唯一一只从没被任何人评过的鼠。**它就是你的检验样本。**

**判定（这条要写进 PR 正文）**：贴出 `30mg_2周-ch4` 那一行的渲染结果，人工六列必须是 `—`。
...
6. `30mg_2周-ch4` 那一行的渲染结果（人工六列必须是 `—` 而不是 `0`）
