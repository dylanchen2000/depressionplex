...
**权威清单在 `pyproject.toml`**（`[project.optional-dependencies].desktop` 与
`[tool.depressionplex.desktop-imports]`）。下面这张表是它的镜像，有守卫逐字对账
（`test_dependency_table_in_spec_matches_pyproject`）——两处分叉即红，不许靠人记（DP-114）。

```
PySide6==6.7.3        # 与 drugeffectscan 同版，踩过的坑不重踩；全外壳可用
openpyxl==3.1.5       # xlsx 导出；**只许出现在 desktop/app/services/export_xlsx.py**
numpy==1.26.4         # 引擎的依赖，不是外壳的（外壳不许 import 引擎）
...
> **通则（道俊 2026-09-13）**：「其他问题如果我们自己没有更好的方案需要决定，可以考虑 CSI 的方案。」
> 用法有个前提：**先去查 CSI 到底怎么做的，查出来的才算 CSI 的方案，猜的不算**。
> 第一次照这条办的是 DP-106（隔间数上界：查了 `.SET`/`.CLB` 两种格式，CSI 都不设上限 ⇒ 我们也不设）。
> 这条不适用于「CSI 那样做但我们知道它错了」的地方（例如 TST 分母用 `image_width`，见 DP-097 §三）。

1. **FST 计分窗口**（DP-057/DP-074）：录像起点是否为入水？还是按每段实长各自定窗？
   这条不定，FST 一侧的所有秒数都不许出。**仍未定。** 上面那条通则在这里要小心用：
   CSI 有它自己的窗口口径，但换窗口会改动 FST 一侧**每一个**报出去的秒数
   ⇒ 属于「有风险的，我解释、道俊决定」那一类，不许 agent 按通则自行照搬。
2. ~~**M2 研究版是否现在就发客户**~~ **已定（2026-09-13）：可以发。** 道俊原话「M2版指的是我们
   即将发布的软件研究版？如果是，可以发」。⇒ 徽章黄是**已被接受的**发布状态，不再是待议项；
   M2 的口径不变（客户机装上、跑完、导出带声明的报告），G11 没有实数门槛这件事必须写在报告声明里。
3. ~~**秒表工具 v1.6（含 seek/逐帧）什么时候换给正在评分的两位同事**~~
   **已完成（2026-09-13 道俊确认）：v1.6 已发给同事，并且已在使用。** 本条原写「他们手上是 v1.4」，
   那是过期信息。⇒ 后续人工评分的真值一律按 v1.6 的口径读；`docs/SOP_悬尾实验人工评分_v1.5.md`
   里关于版本号的说明同步更正。
