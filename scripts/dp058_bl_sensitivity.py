...
#: BL 扰动倍数。±15% 是组内实测漂移的**下限**（`30mg_2周` 组内差 94%），
#: 所以这组倍数是保守的：真实漂移比这更狠。
...
def main() -> int:
    ...
    if rows:
        ...
        if med >= 17.7:
            emit()
            emit("⇒ **BL 是一阶误差源**：仅 ±15%% 的 BL 误差就能单独吃掉整个 G8 门槛，"
                 "而组内实测漂移（`30mg_2周` 达 94%%）远超 ±15%%。"
                 "**下一步：查 BL 为什么在同批组内漂这么多**（首先怀疑裁剪窗口 "
                 "DP-052 让走廊标定落点不同），而不是去调 θ_mob。")
        else:
            emit()
            emit("⇒ **BL 不是一阶误差源**（±15%% 只值 %.1f s < 17.7 s）。"
                 "这条线到此为止，**回去查 DP-052 的裁剪窗口**。"
                 "组内 BL 漂 94%% 仍是缺陷，但它解释不了 35 s 的偏差。" % med)
        emit()
