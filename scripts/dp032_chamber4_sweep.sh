...
#   实测定案：never_occupied=2（v4、v7）。
#   ⚠️ 2026-09-04 更正（DP-043）：当时接着写的"帧级目检两者都是真阴性 ⇒ 无假阳性，DP-032 关闭"
#   **已作废**。其中一个是 `30mg_2周` 的隔间 4，道俊人眼反复确认**那里从头到尾有老鼠**
#   ⇒ 这是 **G10a 假阳性 1**（把有动物的隔间判成空 = 静默丢掉一只真动物），DP-032 已重开。
#   真正从头到尾没有动物的只有 `20mg_3周` 的隔间 4。本脚本的扫描逻辑不变，变的是结论。
#
